...
export async function fetchPokemon(id: number): Promise<Pokemon> {
  const data = await gqlQuery<{ pokemon_v2_pokemon_by_pk: GqlPokemon | null }>(GET_POKEMON, { id });
...
export async function fetchPokemonsByRegion(
  minId: number,
  maxId: number,
  sortBy: SortOption = 'number-asc'
): Promise<PokemonListItem[]> {
...
export async function fetchPokemonsByTypeAndRegion(
  type: string,
  minId: number,
  maxId: number,
  sortBy: SortOption = 'number-asc'
...
export async function fetchFavorites(
  ...
  const variables = typeFilter === 'all' ? { ids, orderBy } : { ids, type: typeFilter, orderBy };
  const data = await gqlQuery<{ pokemon_v2_pokemon: { id: number; name: string }[] }>(
    query,
    variables
  );
